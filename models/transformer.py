...
class StyleTransformer(nn.Module):
    ...
    def forward(
        ...
        """
        Args:
            generate(bool): whether generate or copy a sequence
            differentiable_decode(bool): whether use differentiable decode func
            temperature(float): for softmax with temperature in generate
        """
        ...
        pos_idx = (
            torch.arange(self.max_length).unsqueeze(0).expand((batch_size, -1))
        )
        ...
        tgt_mask = torch.ones((self.max_length, self.max_length)).to(
            src_mask.device
        )
        ...
        if not generate:
            ...
            dec_input_emb = torch.cat(
                (
                    sos_token,
                    self.embed(dec_input, pos_idx[:, : max_dec_len - 1]),
                ),
                1,
            )
...
class DomainDiscriminator(nn.Module):
    def __init__(self, config, vocab):
        ...
        learned_pos_embed = config.learned_pos_embed
        load_pretrained_embed = config.load_pretrained_embed
        num_domain_classes = config.num_domain_classes

        self.pad_idx = vocab.stoi["<pad>"]
        self.domain_embed = Embedding(num_domains, d_model)
        self.embed = EmbeddingLayer(
            vocab=vocab,
            d_model=d_model,
            ...
            load_pretrained_embed=load_pretrained_embed
        )
        self.cls_token = nn.Parameter(torch.randn(d_model))
        self.encoder = Encoder(num_layers, d_model, len(vocab), h, dropout)
        self.classifier = Linear(d_model, num_domain_classes)

    def forward(self, inp_tokens, inp_lengths, domain=None):
        batch_size = inp_tokens.size(0)
        num_extra_token = 1 if domain is None else 2
        max_seq_len = inp_tokens.size(1)
        ...
        if domain is not None:
            domain_emb = self.domain_embed(domain).unsqueeze(1)
            enc_input = torch.cat((enc_input, domain_emb), 1)
...
class Decoder(nn.Module):
    def __init__(self, num_layers, d_model, vocab_size, h, dropout):
        ...
        self.layers = nn.ModuleList(
            [DecoderLayer(d_model, h, dropout) for _ in range(num_layers)]
        )
        self.norm = LayerNorm(d_model)
...
class DecoderLayer(nn.Module):
    ...
    def incremental_forward(
        self, x, memory, src_mask, tgt_mask, prev_states=None
    ):
        new_states = []
        m = memory
