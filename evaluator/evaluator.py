...
class Evaluator(object):
    ...
    def nltk_bleu(self, texts_origin, text_transfered):
        texts_origin = [
            word_tokenize(text_origin.lower().strip())
    ...
    def yelp_ref_bleu_0(self, texts_neg2pos):
        assert (
            len(texts_neg2pos) == 500
        ), "Size of input differs from human reference file(500)!"
        sum = 0
        n = 500
    ...
    def yelp_ref_bleu(self, texts_neg2pos, texts_pos2neg):
        ...
        n = 1000
        for x, y in zip(
            self.yelp_ref[0] + self.yelp_ref[1], texts_neg2pos + texts_pos2neg
        ):
            sum += self.nltk_bleu([x], y)
        return sum / n
    ...
    def yelp_ppl(self, texts_transfered):
        texts_transfered = [
            " ".join(word_tokenize(itm.lower().strip()))
            for itm in texts_transfered
        ]
        sum = 0
        words = []
        length = 0
