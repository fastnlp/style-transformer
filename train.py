...
def batch_preprocess(
    batch, pad_idx, eos_idx, domain: Optional[str] = None, reverse=False
):
    # TODO: preprocess batch to add domain info
    ...
    if domain:
        domains = (
            torch.ones_like(styles)
            if domain == "yelp"
            else torch.zeros_like(styles)
        )
    else:
        domains = None

    return tokens, lengths, styles, domains
...
def d_step(config, vocab, model_F, model_D, optimizer_D, batch, temperature):
    ...
    inp_tokens, inp_lengths, raw_styles, _ = batch_preprocess(
...
def domain_d_step(
    ...
    assert domain == "yelp" or domain == "imdb"
    ...
    inp_tokens, inp_lengths, raw_styles, domains = batch_preprocess(
        batch,
        pad_idx,
        eos_idx,
        domain=domain,
    ...
    optimizer_domain_D.zero_grad()
    loss.backward()
    clip_grad_norm_(model_domain_D.parameters(), 5)
    optimizer_domain_D.step()

    model_F.train()

    return domain_cns_loss.item()
...
def f_step(
    ...
    drop_decay,
    cyc_rec_enable=True,
    model_domain_D: Optional[nn.Module] = None,
    domain: Optional[str] = None,
):
    assert config.use_domain_adaption==False or (config.use_domain_adaption and model_domain_D and domain)
    model_D.eval()
    if model_domain_D:
        model_domain_D.eval()
    ...
    noise_inp_tokens = word_drop(
        inp_tokens, inp_lengths, config.inp_drop_prob * drop_decay, vocab
    )
    noise_inp_lengths = get_lengths(noise_inp_tokens, eos_idx)

    slf_log_probs = model_F(
        inp_tokens=noise_inp_tokens,
        gold_tokens=inp_tokens,
        inp_lengths=noise_inp_lengths,
        style=raw_styles,
        generate=False,
    ...
    slf_rec_loss = (
        loss_fn(slf_log_probs.transpose(1, 2), inp_tokens) * token_mask
    )
    ...
    if not cyc_rec_enable:
        ...
        model_D.train()
        if model_domain_D:
            model_domain_D.train()
        return slf_rec_loss.item(), 0, 0, 0
    ...
    gen_soft_tokens = gen_log_probs.exp()  # transfered tokens
    ...
    cyc_rec_loss = (
        loss_fn(cyc_log_probs.transpose(1, 2), inp_tokens) * token_mask
    )
    ...
    if model_domain_D:
        ...
        domain_cns_loss = loss_fn(domain_cns_log_probs, domains)
        domain_cns_loss = domain_cns_loss.sum()
        domain_cns_loss *= config.domain_cns_factor
        loss += domain_cns_loss
    else:
        domain_cns_loss = torch.tensor(0.0, requires_grad=False)
...
def train(
    ...
    assert config.use_domain_adaption==False or (config.use_domain_adaption and model_domain_D)

    optimizer_F = optim.Adam(
        model_F.parameters(), lr=config.lr_F, weight_decay=config.L2
    )
    optimizer_D = optim.Adam(
        model_D.parameters(), lr=config.lr_D, weight_decay=config.L2
    )
    if model_domain_D:
        optimizer_domain_D = optim.Adam(
            model_domain_D.parameters(),
            lr=config.lr_domain_D,
    ...
    while True:
        ...
        # TODO: update domain discriminator
        for _ in range(config.iter_D):
            ...
            if model_domain_D:
                d_domain_cns_loss = domain_d_step(
                    config,
                    vocab,
                    model_F,
                    model_domain_D,
        ...
        for _ in range(config.iter_F):
            ...
            if model_domain_D:
                ...
                domain = None
            f_slf_loss, f_cyc_loss, f_adv_loss, f_domain_cns_loss = f_step(
                config,
                vocab,
                model_F,
                model_D,
        ...
        if global_step % config.eval_steps == 0:
            ...
            his_f_cyc_loss = []
            his_f_adv_loss = []
            his_f_domain_cns_loss = []

            # save model
            torch.save(
                model_F.state_dict(),
                config.save_folder + "/ckpts/" + str(global_step) + "_F.pth",
            )
            torch.save(
                model_D.state_dict(),
                config.save_folder + "/ckpts/" + str(global_step) + "_D.pth",
            )
            auto_eval(
                config, vocab, model_F, test_iters, global_step, temperature
            )
